...
const jsonFiles = [
    ...
    { file: "../py/년간.json", type: "bar", flip: 0 }        
];
...
function formatNumber(value, columnName = "") {
    ...
    if (columnName) {                                       // 컬럼명이 전달된 경우
        const col = columnName.trim();                      // 공백 제거
    ...
    if (typeof value === "number" || !isNaN(value)) {       // 숫자 또는 숫자형 문자열인지 확인
        const num = parseFloat(value);                      // 숫자 변환
        const str = value.toString();                       // 문자열로 변환

        // --- ③ 예외 1: 4자리 연도 ----------------------------------------
        if (Number.isInteger(num) && num >= 1000 && num <= 9999) {
            return str;                                     // 4자리 연도는 그대로 반환
        }

        // --- ④ 예외 2: 연도.월 형태 ----------------------------------------
        if (/^\d{4}\.\d{1,2}$/.test(str)) {                 // 2020.01 형태인지 정규식 검사
            return str;                                     // 그대로 반환
        }
        ...
        if (num % 1 === 0) {                                // 정수인 경우
            ...
            return num.toLocaleString("ko-KR", {            // 천 단위 + 소수점 자리 지정
